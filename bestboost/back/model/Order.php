...
class Order
{
	...
	//NetWin
	public function addOrderNetWin($dbcon, $type, $current_rank, $number_games, $server, $duo_boost, $roles_champions, $twitch_stream, $h_priority, $express_order, $primary_role, $primary_champion, $secondary_role, $secondary_champion, $total_amount, $user_id, $status)
	{
		$sql = "INSERT INTO OrdersNetWin (type, current_rank, number_games, server, duo_boost, roles_champions, twitch_stream, h_priority, express_order, primary_role, primary_champion, secondary_role, secondary_champion, total_amount, user_id, status, date)
			VALUES (:type, :current_rank, :number_games, :server, :duo_boost, :roles_champions, :twitch_stream, :h_priority, :express_order, :primary_role, :primary_champion, :secondary_role, :secondary_champion, :total_amount, :user_id, :status, NOW())";

			$pst = $dbcon->prepare($sql);
			$pst->bindParam(':type', $type);
			$pst->bindParam(':current_rank', $current_rank);
			$pst->bindParam(':number_games', $number_games);
			$pst->bindParam(':server', $server);
			$pst->bindParam(':duo_boost', $duo_boost);
			$pst->bindParam(':roles_champions', $roles_champions);
			$pst->bindParam(':twitch_stream', $twitch_stream);
	...
	public function getPriceByRangeNetWin($dbcon, $current_rank_netwin)
	{
		$sql = "SELECT price FROM PricingNetWin
				WHERE current_rank_netwin = :current_rank_netwin";
		$pst = $dbcon->prepare($sql);
		$pst->bindParam(':current_rank_netwin', $current_rank_netwin);
        $pst->execute();
